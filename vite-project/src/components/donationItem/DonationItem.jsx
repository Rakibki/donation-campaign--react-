  ...
  return (
    <div style={{backgroundColor: `${item.card_bg_color}`}} className='flex  md:items-center rounded-md'>
        ...
        <div className='p-4'>
            ...
            <Link to={`/card/${item.id}`}><button className='mt-2 text-sm font-semibold text-white px-3 py-2 rounded-md' style={{backgroundColor: `${item.text_color}`}}>View Details</button></Link>
        </div>
    </div>
  )
}
